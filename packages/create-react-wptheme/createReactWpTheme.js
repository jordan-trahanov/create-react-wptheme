...
const scriptsFromGit = function () {
  ...
  let scriptsPath =
    "file:" +
    path.join(tempPath, "create-react-app", "packages", "react-scripts");
  return {
    path: scriptsPath,
    callback: function () {
...
const program = new commander.Command(packageJson.name)
  ...
  .option(
    "--use-npm",
    "force downloading packages using npm instead of yarn (if both are installed)"
  )
  .option("--use-pnp")
...
function createApp(
  ...
  console.log(
    `Using Create React App ${chalk.green(
      _createReactAppVersion
    )} to scaffold the theme's source code...`
  );
  console.log();
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(command, args, { stdio: "inherit" })
      .on("error", function (err) {
        console.log(
          `createReactWpTheme.js ERROR for command: ${command} ${args.join(
            " "
          )}`
        );
        throw err;
