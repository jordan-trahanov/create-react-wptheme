...
const _getUserConfig = require("@canufeelit/create-react-wptheme-utils/getUserConfig");
...
const wpThemeServer = {
  getClientInjectString: function (mode, token) {
    ...
    const jsTags = [
      "<script src='<?php echo $BRC_TEMPLATE_PATH; ?>/react-src/node_modules/@canufeelit/create-react-wptheme-utils/wpThemeClient.js'></script>\n",
      "<script src='<?php echo $BRC_TEMPLATE_PATH; ?>/react-src/node_modules/@canufeelit/create-react-wptheme-utils/wpThemeErrorOverlay.js'></script>\n",
    ];
    ...
    switch (mode) {
      ...
      default:
        console.log(
          chalk.magenta(
            `wpstart::injectWpThemeClient: unknown inject mode: ${mode}.`
          )
        );
        console.log(
          `Available inject modes: ${chalk.cyan(
            "disable, afterToken, beforeToken, replaceToken, endOfFile"
          )}`
        );
        process.exit();
    }
  ...
  startServer: function (paths) {
    try {
      ...
      console.log(
        "unable to get wpThemeServer config from user config. Err:",
        err
      );
      process.exit(1);
    }
    ...
    _configHost =
      _serverConfig &&
      typeof _serverConfig.host === "string" &&
      _serverConfig.host.length > 0
        ? _serverConfig.host
        : "127.0.0.1";
    _configPort =
      _serverConfig && typeof _serverConfig.port === "number"
        ? _serverConfig.port
        : null;
    ...
    if (_serverPort > 0) {
      _configSslCert =
        _serverConfig &&
        typeof _serverConfig.sslCert === "string" &&
        _serverConfig.sslCert.length > 0
          ? _serverConfig.sslCert
          : null;
      _configSslKey =
        _serverConfig &&
        typeof _serverConfig.sslKey === "string" &&
        _serverConfig.sslKey.length > 0
          ? _serverConfig.sslKey
          : null;

      if (
        typeof _configSslCert === "string" &&
        typeof _configSslKey === "string"
      ) {
    ...
    if (stats) {
      if (typeof stats.hasErrors === "undefined") {
        ...
        if (typeof stats.hasErrors === "function " && stats.hasErrors()) {
          ...
          typeof stats.hasWarnings === "function " &&
          stats.hasWarnings()
        ) {
          _sendMessage(_typeBuildWarning, stats);
        } else {
