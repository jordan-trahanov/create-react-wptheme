...
const wpThemeUserConfig = require("@canufeelit/create-react-wptheme-utils/getUserConfig"); //(paths, process.env.NODE_ENV);
...
const fileFunctions = {
  copyPublicFolder: function (paths) {
    fs.copySync(paths.appPublic, paths.appBuild, {
      ...
      filter: (file) =>
        ...
        file.indexOf(wpThemePostInstallerInfo.postInstallerName) == -1,
    });
  },
  copyToThemeFolder: function (paths) {
    ...
    let actionPath =
      (forBuild && userConfig && userConfig.finalBuildPath) || "..";
    ...
    if (forBuild === true) {
      ...
      actionPath =
        (userConfig && userConfig.finalBuildPath) || path.join("..", themeName);
