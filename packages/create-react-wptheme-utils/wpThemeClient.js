...
var wpThemeClient = {
  ...
  start: function (wsHostProtocol, wsHostname, wsPort) {
    ...
    switch (wsHostProtocol) {
      ...
      default:
        console.log(
          `WPTHEME CLIENT: configHostProtocol is not "ws" or "wss": ` +
            new String(wsHostProtocol)
        );
        console.error(
          "This is a bug. Please report to: https://github.com/jordan-trahanov/create-react-wptheme/issues"
        );
        return;
    }
    ...
    if (wsHostname !== "__from-window__") {
      if (typeof wsHostname !== "string" && wsHostname.length <= 0) {
        console.log(
          "WPTHEME CLIENT: hostname is not '__from-window__' or a non-empty string: ",
          wsHostname
        );
        return;
      }
    ...
    if (wsPort !== "__from-window__") {
      ...
      if (typeof parsedConfigPort !== "number") {
        console.log(
          "WPTHEME CLIENT: port is not '__from-window__' or a number: ",
          wsPort
        );
        return;
      }
    ...
    var hostName =
      wsHostname === "__from-window__" ? window.location.hostname : wsHostname;
    var portNum =
      wsPort === "__from-window__" ? window.location.port : parsedConfigPort;
    ...
    wpThemeClient.socket.onmessage = function (response) {
      if (response && typeof response.data === "string") {
          ...
          if (msg) {
            ...
            switch (msg.type) {
              case "content-changed":
                if (
                  !newlyReloaded ||
                  wpThemeClient.hash === null ||
                  (typeof msgHash === "string" &&
                    msgHash.length > 0 &&
                    msgHash !== wpThemeClient.hash)
                ) {
              ...
              case "hash-check":
                if (wpThemeClient.hash === null) {
                  ...
                  !newlyReloaded &&
                  typeof wpThemeClient.hash === "string" &&
                  wpThemeClient.hash !== msgHash
                ) {
                  window.location.reload();
                }
        ...
        newlyReloaded = false;
        wpThemeClient.hash =
          typeof msgHash === "string" && msgHash.length > 0 ? msgHash : null;
      }
    };
      ...
      if (console && typeof console.info === "function") {
        switch (wpThemeClient.socket.readyState) {
          ...
          case wpThemeClient.socket.CLOSING:
            setTimeout(() => {
              console.info(
                "It's possible the browser refresh server has disconnected.\nYou can manually refresh the page if necessary."
              );
            }, 1000);
            break;
        }
